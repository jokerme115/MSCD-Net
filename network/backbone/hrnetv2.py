        ...
        for branch_output_index in range(
            ...
            for input_index in range(self.number_of_branches):  # The inputs of other branches to be fused.
                if input_index == 0:
                    ...
                    fused_tensor = x_fused[branch_output_index]
                    source_tensor = self.fuse_layers[branch_output_index][input_index](x[input_index])
                    
                    # 检查并调整尺寸以确保匹配
                    if fused_tensor.size()[2:] != source_tensor.size()[2:]:
                        # 使用双线性插值调整source_tensor尺寸以匹配fused_tensor
                        source_tensor = F.interpolate(
                            source_tensor, 
                            size=fused_tensor.size()[2:], 
                            mode='bilinear', 
                            align_corners=False
                        )
                    
                    x_fused[branch_output_index] = fused_tensor + source_tensor
...
class HRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Stage 1
        x = self.layer1(x)
        x_list = [trans(x) for trans in self.transition1]  # split to 2 branches, form a list.
        ...
        output_h, output_w = x_list[0].size(2), x_list[0].size(3)  # Upsample to size of highest resolution stream
